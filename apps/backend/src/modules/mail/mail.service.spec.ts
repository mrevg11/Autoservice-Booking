...
describe('MailService', () => {
  ...
  it('sendBookingConfirmed: does not throw', async () => {
    const ctx = {
      bookingId: 1, clientName: 'Клієнт', masterName: 'Майстер',
      services: 'ТО', scheduledAt: '01 травня 2026', duration: 60,
      totalPrice: '500.00', bookingUrl: 'http://localhost/booking/1',
    };
  ...
  it('sendBookingCancelled: does not throw', async () => {
    const ctx = {
      bookingId: 2, clientName: 'Клієнт', masterName: 'Майстер',
      services: 'ТО', scheduledAt: '01 травня 2026', duration: 60,
      totalPrice: '500.00', bookingUrl: 'http://localhost/booking/2', reason: 'Тест',
    };
    await expect(service.sendBookingCancelled('to@test.com', ctx)).resolves.not.toThrow();
  });

  it('sendStatusChanged: does not throw', async () => {
    await expect(service.sendStatusChanged('to@test.com', {
      bookingId: 3, clientName: 'Клієнт', newStatus: 'Підтверджено',
      scheduledAt: '01 травня 2026', bookingUrl: 'http://localhost/booking/3',
    })).resolves.not.toThrow();
  });

  it('sendBookingReminder24h: does not throw', async () => {
    const ctx = {
      bookingId: 4, clientName: 'Клієнт', masterName: 'Майстер',
      services: 'ТО', scheduledAt: '02 травня 2026', duration: 60,
      totalPrice: '400.00', bookingUrl: 'http://localhost/booking/4',
    };
  ...
  it('sendBookingReminder2h: does not throw', async () => {
    const ctx = {
      bookingId: 5, clientName: 'Клієнт', masterName: 'Майстер',
      services: 'ТО', scheduledAt: '02 травня 2026', duration: 60,
      totalPrice: '400.00', bookingUrl: 'http://localhost/booking/5',
    };
